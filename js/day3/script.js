...
const employeesDataArray = JSON.parse(employeesData);
...
const bonus = employeesDataArray.filter(employee => Number(employee.hr.salary.replace(/[^\w]/g, '')) <= 100000)
    .map(employee => ({
      ...employee,
      hr: { ...employee.hr, 
        bonus: `${Number(employee.hr.salary.replace(/[^\w]/g, '')) * 20 / 100}` 
      } }));
console.log(bonus);
...
const numberEmployeesByCity = Object.assign(...Array.from(cities, cityName => ({ [cityName]: '' })));

for (let i in employeesDataArray) {
  if (cities.has(employeesDataArray[i].contact[0])) {
...
setTimeout(() => {
    const employeesNames = employeesDataArray.map((key) => key.name);
    console.log('All our employees:', employeesNames.join(', '))

    const removeEmployee = employeesNames[Math.floor(Math.random() * (employeesNames.length))];
    console.log('You are fired:', removeEmployee);

    const employeesList = employeesNames.filter((name) => name !== removeEmployee);

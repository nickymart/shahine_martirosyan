...
for(let i in employeesDataArray){
  if(employeesDataArray[i].hr['salary'].replace(/[^\w]/g, '') <= 100000){
    employeesDataArray[i].hr['bonus'] = employeesDataArray[i].hr['salary'].replace(/[^\w]/g, '') * 20 / 100;
    bonus[j] = employeesDataArray[i];
    j++;
  }
}
console.log(bonus);
...
for (let i in employeesDataArray) {
  if(numberEmployeesByCity.hasOwnProperty(employeesDataArray[i].contact[0]))
    numberEmployeesByCity[employeesDataArray[i].contact[0]]++;
  else
    numberEmployeesByCity[employeesDataArray[i].contact[0]] = 1;
}
console.log(numberEmployeesByCity);

// 6

console.log('BOSS: Wait');

setTimeout(() => {
  let employeesName = new Array();
  for(const i in employeesDataArray) {
    employeesName[i] = employeesDataArray[i]['name'];
  }
  console.log('All our employees:', employeesName.join(', '))
  
  const removeEmployee = employeesName[Math.floor(Math.random() * (employeesName.length))];
  console.log('You are fired:', removeEmployee);
  for(let i in employeesDataArray) {
    if(employeesDataArray[i]['name'] === removeEmployee) {
      employeesDataArray.splice(i, 1);
    }
  }
  
  let employeesList = new Array();
  for(const i in employeesDataArray) {
    employeesList[i] = employeesDataArray[i]['name'];
  }
  console.log('List of new employees։', employeesList.join(', '));
}, 3000);
